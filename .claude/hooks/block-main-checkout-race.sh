#!/bin/bash
# block-main-checkout-race.sh — PreToolUse Bash hook
#
# When more than one live Sonde Claude Code session is active, branch / HEAD /
...
# feature work per ADR 0002 (docs/adr/0002-git-workflow-and-governance.md).
#
# Leases live at <git-common-dir>/session-leases/ — i.e., shared by the main
# checkout AND every worktree of this repo. (A worktree's .claude/session-leases/
# would be a different filesystem location from main's, so leases written from a
# worktree never propagate to main; storing them under the shared git common-dir
# fixes the cross-checkout visibility gap.)
#
# Side effects:
...
#
# Ported from tuxlink/.claude/hooks/block-main-checkout-race.sh.
...
# Leases MUST live in a location shared by main + all worktrees, not per-checkout
# under .claude/. `git rev-parse --git-common-dir` returns the shared .git/
# regardless of which checkout we're invoked from.
...
# --- Detect git-target-override forms ---
...
moniker="${SONDE_AGENT_MONIKER:-(unknown)}"
...
#    the command targets only my worktree; let it through. This fast-path is
#    unsafe when `git -C` / `--git-dir` redirects target — fall through to the
#    lease check in that case.
...
# all local branches) and the explicit listing forms.
...
Another live Sonde session is active: $other_summary.
This session does NOT own the main-checkout lease.

Per ADR 0002 (docs/adr/0002-git-workflow-and-governance.md): worktrees are
...
ownership on every invocation. See docs/git-strategy.md.
