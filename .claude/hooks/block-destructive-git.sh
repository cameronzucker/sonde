...
# per CLAUDE.md §"Git workflow — destructive commands are BANNED" and
...
#
# Ported from tuxlink/.claude/hooks/block-destructive-git.sh.
...
# documented in docs/git-strategy.md. `git worktree remove` direct-unlinks the worktree, bypassing
# any trash/recycle safety net and silently discarding gitignored-but-stateful content (e.g.
# .beads/embeddeddolt/ or a worktree-local target/) that the "looks clean" check misses.
if printf '%s' "$cmd" | grep -qE '\bgit[[:space:]]+worktree[[:space:]]+remove\b'; then
    deny "git worktree remove is banned. Use the disposal ritual in docs/git-strategy.md: inventory ('git status --short' + 'git ls-files --others --exclude-standard' + 'git ls-files --others --ignored --exclude-standard') -> archive if needed -> cd back to the main repo -> rm -rf <worktree-path> -> git worktree prune."
fi
