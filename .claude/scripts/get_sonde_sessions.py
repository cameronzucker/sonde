#!/usr/bin/env python3
"""get_sonde_sessions.py — list live Sonde Claude Code sessions in this repo.
...
argue with a hook deny.

Usage:
  .claude/scripts/get_sonde_sessions.py
  .claude/scripts/get_sonde_sessions.py --ttl-minutes 60
  .claude/scripts/get_sonde_sessions.py --include-stale

Ported from tuxlink/.claude/scripts/get_tuxlink_sessions.py.
"""
...
def resolve_lease_dir(repo: Path) -> Path:
    ...
    so script and hook agree on what is and isn't a live lease. The
...
def main() -> int:
    ...
    if not lease_dir.exists():
        print("No active Sonde sessions (lease directory does not exist yet).")
        return 0
    ...
    if not sessions:
        print("No live Sonde sessions in this repo.")
        if main_holder:
    ...
    print()
    print("Active Sonde sessions in this repo:")
