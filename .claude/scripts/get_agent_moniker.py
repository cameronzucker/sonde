...
Pre-flights against `git log --all --grep="^Agent: <candidate>"` — if a
collision is detected, retries up to --max-attempts times before giving up.
...
Ported from tuxlink/.claude/scripts/get_agent_moniker.py.
"""
...
def resolve_repo() -> Path:
    ...
    repo. Otherwise falls back to the script-relative path (../..). A
    misconfigured CLAUDE_PROJECT_DIR would otherwise silently make git log fail,
    and moniker_taken would treat the failure as "no collision" — letting
    duplicate monikers through. The fallback makes a non-git CLAUDE_PROJECT_DIR
...
def main() -> int:
    ...
    for attempt in range(1, args.max_attempts + 1):
        ...
        if taken is None:
            ...
            # Fail closed — refuse to ship an unchecked moniker.
            sys.stderr.write(
