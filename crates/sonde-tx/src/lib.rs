...
//!
//! Phase 3 of the sonde hardware bring-up (tuxlink-i3bz / tuxlink-9ggl).
//! Composes three already-shipped layers:
//!
//! - [`sonde_phy::robustness_floor::wideband_lowdensity::WidebandLowDensityFloor`] — the PHY encoder
//! - [`sonde_phy::audio_device::AudioOutput`] — real-time soundcard output
//! - [`sonde_rig_rts::RtsPtt`] — Digirig + G90 PTT primitive (serial-RTS)
//!
...
//! `bin/sonde-tx.rs` wires the orchestration to the Linux-specific
...
//!
//! [`LinuxTty`]: sonde_rig_rts::LinuxTty
//! [`AudioOutput`]: sonde_phy::audio_device::AudioOutput
...
/// [`sonde_phy::audio_device::AudioOutput::play_blocking_with_abort`].
...
impl Mode {
    ...
    /// the PHY's [`sonde_phy::modes::ModeDescriptor::short_name`]
...
pub enum FrameMode {
    ...
    /// (~9 bytes for the Wide mode). Pairs with `sonde-rx --frame-mode sync`.
    Sync,
    /// Zadoff-Chu preamble + N OFDM symbols carrying a 2-byte length-
    /// prefix header. Supports arbitrary-length payloads up to
    /// u16::MAX bytes. Pairs with `sonde-rx --frame-mode multi-sync`.
    MultiSync,
}
...
/// [`sonde_phy::audio_device::AudioOutput`]. Lets
...
impl AbortablePlay for sonde_phy::audio_device::AudioOutput {
    fn play_blocking_with_abort(
        &mut self,
        buffer: &AudioBuffer,
        abort: &AtomicBool,
...
pub struct Args {
    ...
    /// Spawn `sonde-rig-watchdog` as a child process to hold PTT (Phase 1.5
    ...
    pub watchdog: bool,
    /// Optional explicit path to the `sonde-rig-watchdog` binary. When
    /// `None` and `watchdog` is set, the spawn falls back to looking up
    /// `sonde-rig-watchdog` on `PATH`.
...
mod tests {
    ...
    fn resolve_payload_at_prefix_reads_file() {
        ...
        let path = dir.join(format!("sonde-tx-test-payload-{}", std::process::id()));
    ...
    fn args_parse_watchdog_bin_with_path() {
        ...
        assert_eq!(
            ...
            Some(std::path::Path::new("/usr/local/bin/sonde-rig-watchdog"))
        );
    }
