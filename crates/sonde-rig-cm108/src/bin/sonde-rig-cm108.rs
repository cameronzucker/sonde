//! sonde-rig-cm108 — CLI for the CM108 PTT primitive.
//!
...
//!
//! ```text
//! sonde-rig-cm108 toggle  --device <path>                — assert then immediately release
//! sonde-rig-cm108 assert  --device <path> --duration <s> — assert for at most N seconds
//! sonde-rig-cm108 release --device <path>                — release explicitly
//! ```
//!
//! ## Safety
//!
...
use sonde_rig_cm108::{Cm108Error, GpioPin, HidrawWriter, Ptt, PttState};

// Wrap the generic Cm108Ptt at the binary boundary with the real
// hidraw writer.
type Cm108 = sonde_rig_cm108::writer::Cm108Ptt<HidrawWriter>;
...
const USAGE: &str = "\
sonde-rig-cm108 — CM108-family USB-HID PTT control

USAGE:
    sonde-rig-cm108 <COMMAND> --device <PATH> [OPTIONS]
...
EXAMPLES:
    ...
    sonde-rig-cm108 toggle --device /dev/dra100-ptt

    # Key the radio for up to 5 seconds (Ctrl+C aborts early + releases):
    sonde-rig-cm108 assert --device /dev/dra100-ptt --duration 5
