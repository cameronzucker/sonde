//! sonde-rx — capture + demod + BER composition.
//!
//! Phase 4 of the sonde hardware bring-up (tuxlink-xvrb / tuxlink-9ggl).
//! Receive complement to [tuxlink-i3bz] (sonde-tx). Composes the
//! capture side of [`sonde_phy::audio_device::AudioInput`] with the
...
//! does NOT gate this binary the way it gates `sonde-tx`; agents
...
//! [`WidebandLowDensityFloor::receive`]: sonde_phy::robustness_floor::wideband_lowdensity::WidebandLowDensityFloor::receive
...
impl Mode {
    ...
    /// sonde-tx-facing name) and `floor-wblo` (the PHY's
...
/// Frame format selection. Mirrors `sonde-tx`'s `FrameMode` —
...
// ─── Expected-payload resolution (mirrors sonde-tx::resolve_payload) ──

/// Resolve an `--expected` argument value to its byte sequence. Same
/// two forms as `sonde-tx`'s `--payload`: bare text (UTF-8 bytes)
...
mod tests {
    ...
    fn resolve_expected_at_prefix_reads_file() {
        ...
        let path = dir.join(format!("sonde-rx-test-exp-{}", std::process::id()));
    ...
    fn roundtrip_wide_floor_encode_then_decode_recovers_payload() {
        // The headline acceptance test: sonde-tx's encoder + our
    ...
    fn decode_sync_roundtrip_via_transmit_with_preamble() {
        // The headline sync-side acceptance test: sonde-phy's
    ...
    fn end_to_end_multi_sync_roundtrip_via_wav_file() {
        ...
        let dir = std::env::temp_dir();
        let path = dir.join(format!(
            "sonde-rx-multi-sync-roundtrip-{}.wav",
            std::process::id()
        ));
        buffer.write_wav(&path).unwrap();
        let read_back = read_wav(&path).unwrap();
