//! sonde-rx — capture + demod + BER CLI (Phase 4, tuxlink-xvrb).
//!
...
//! this binary the way it gates `sonde-tx`.
...
const USAGE: &str = "\
sonde-rx — capture + demod + BER (Phase 4, tuxlink-xvrb)

USAGE:
    sonde-rx --list-devices
        Enumerate CPAL input devices and their 48 kHz f32 support.

    sonde-rx --decode-wav <PATH> [--mode <NAME>] [--expected <ARG>]
        Read a WAV (48 kHz f32 mono); decode the first OFDM-symbol
        worth of samples; print bytes (+ BER vs --expected if given).

    sonde-rx --record-wav <PATH> --device <NAME> --duration <SECS>
...
EXAMPLES:
    ...
    sonde-rx --decode-wav captured.wav --expected \"TEST\"

    # Decode a multi-symbol payload with preamble alignment:
    sonde-rx --decode-wav off-air.wav --expected \"LONG-MESSAGE\" \\
                --frame-mode multi-sync

    # Capture 10 s of audio from the Digirig's input:
    sonde-rx --record-wav off-air.wav --device 'USB Audio Device' --duration 10
