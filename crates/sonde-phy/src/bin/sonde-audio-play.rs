//! sonde-audio-play — CLI for the PHY's real-time audio output.
//!
//! Operator's bench tool for validating that the audio chain from
//! sonde-phy to the soundcard (Digirig USB audio + onward to the
...
//!
//! sonde-audio-play --device <name> --sine <hz>:<seconds>
...
//!
//! ```sh
//! cargo run --manifest-path sonde/crates/sonde-phy/Cargo.toml \
//!   --features audio-device --bin sonde-audio-play -- --list
//! ```
//!
//! ## Safety
//!
...
use sonde_phy::audio_device::{list_output_devices, AudioOutput};
use sonde_phy::audio_io::{AudioBuffer, SAMPLE_RATE_HZ};
use sonde_phy::PhyError;
...
const USAGE: &str = "\
sonde-audio-play — bench tool for the sonde PHY's real-time audio output
...
EXAMPLES:
    ...
    sonde-audio-play --device 'USB Audio Device' --sine 1000:3
...
SAFETY:
    ...
    sonde-rig-rts in a separate terminal BEFORE running this — and
